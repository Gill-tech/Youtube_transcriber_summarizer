...
from utils import extract_transcript, generate_keywords, children_reccomendation
from youtube_id_extractor import extract_video_id
from prefect import flow


st.title("YouTube Video Children Recommender")

@flow(name='Children_Recommender')
def main():
    # URL entry
    youtube_url = st.text_input("Enter the youtube video URL or link")

    # show the user the summary of the video
    st.write("Click the button below to get the recommendation of the video for children")
    if st.button("Recommend for Children"):
            ...
            if youtube_id is None or youtube_id.strip() == "":
                ...
                if transcript == "Error":
                    ...
                    with st.spinner("Generating Recommendation..."):
                        ...
                        try:
                            summary = children_reccomendation(transcript)
